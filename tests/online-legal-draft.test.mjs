// Checks on the retained, unlaunched online-service privacy policy and terms. These pages are what App Store
...
// Render the actual draft components without making them public routes. This keeps
// the detailed online disclosure checks while the released site describes today's
// on-device service. The production placeholder guard is tested separately below.
import { build } from "esbuild";
import { mkdtempSync, writeFileSync, rmSync } from "node:fs";
import { tmpdir } from "node:os";
import { join } from "node:path";
import { createRequire } from "node:module";
const temp = mkdtempSync(join(tmpdir(), "snaglist-legal-draft-"));
const fixture = await build({
  stdin: {
    contents: `import React from 'react';
    import { renderToStaticMarkup } from 'react-dom/server';
    import { Privacy } from './pages/PrivacyOnlineDraft';
    import { Terms } from './pages/TermsOnlineDraft';
...
const fixturePath = join(temp, "fixture.cjs");
writeFileSync(fixturePath, fixture.outputFiles[0].contents);
const { render } = createRequire(import.meta.url)(fixturePath);
const originalIndexing = process.env.INDEX_PUBLIC_SITE;
process.env.INDEX_PUBLIC_SITE = "false";
const draftHTML = render();
if (originalIndexing === undefined) delete process.env.INDEX_PUBLIC_SITE;
else process.env.INDEX_PUBLIC_SITE = originalIndexing;
rmSync(temp, { recursive: true, force: true });
const html = (path) => draftHTML[path];
...
function documentText(page) {
  ...
  assert.ok(
    start > 0 && end > start,
    `${page}: legal document not found in the build`,
  );
  return source
    .slice(start, end)
    .replace(/<!--[\s\S]*?-->/g, "")
...
function metaDescription(page) {
  const match = readFileSync(`build/client${page}/index.html`, "utf8").match(
...
test("legal pages use no banned or pre-release wording", () => {
  for (const page of legalPages) {
    ...
    for (const [pattern, reason] of banned)
      assert.ok(
        !pattern.test(text),
        `${page} ${reason}: ${text.match(pattern)?.[0]}`,
      );
  }
});
  ...
  for (const page of legalPages) {
    ...
    for (const [pattern, reason] of banned)
      assert.ok(
        ...
        `${page} description ${reason}: ${description}`,
      );
  }
});
...
test("the deletion disclosure says what the request does and makes no absolute promise", () => {
  ...
  assert.ok(
    !/\b(?:erased|deleted|removed)\s+immediately\b|\bimmediately\s+(?:erased|deleted|removed)\b/i.test(
      text,
    ),
    "an absolute immediate-erasure claim has come back",
  );
  assert.ok(
    !/\bevery deletion within\b/i.test(text),
    "the universal deletion guarantee has come back",
  );
  for (const [sentence] of text.matchAll(/[^.]*\bwithin 30 days\b[^.]*\./gi))
    assert.match(
      sentence.trim(),
      /^We aim to complete background deletion within 30 days\b/,
      `unconditional 30-day promise: ${sentence.trim()}`,
    );
});
...
test("the rights section states the statutory response period and how an extension works", () => {
  const text = documentText("/privacy");
  assert.ok(
    text.includes("within one calendar month of receiving your request"),
    "one-month response period missing",
  );
  assert.ok(
    text.includes(
      "we may take up to two further months; if so, we will tell you within the first month and explain why",
    ),
    "extension explanation missing",
  );
  assert.ok(
    !/withdraw consent/i.test(text),
    "no processing this policy describes relies on consent",
  );
});
...
test("the photo disclosure says originals are kept and who can download them", () => {
  ...
  for (const sentence of [
    ...
    assert.ok(text.includes(sentence), `missing: ${sentence}`);
  assert.ok(
    !/is ever shown to anyone/i.test(text),
    "the superseded “never shown” photo claim has come back",
  );
});
...
test("the policy states Snaglist’s role, a legal basis per purpose and non-absolute provider wording", () => {
  const text = documentText("/privacy");
  assert.ok(
    text.includes("For that information we act as a data controller."),
    "role paragraph missing",
  );
  assert.ok(
    text.includes(
      "we process those records on its instructions to provide the service",
    ),
    "processor role missing",
  );
  for (const basis of ["contract", "legitimate interests", "legal obligation"])
    assert.ok(text.includes(basis), `legal basis missing: ${basis}`);
  assert.ok(
    !/process information only on our instructions/i.test(text),
    "absolute provider wording has come back",
  );
  assert.ok(
    text.includes("protect it at least as well as this policy does"),
    "equal-protection statement (5.1.1(i)) missing",
  );
  assert.ok(
    text.includes("for its own purposes"),
    "providers’ own purposes not acknowledged",
  );
});

// The provider's identity and the publication date each come from one source in
// pages/LegalPlaceholder.tsx, so the two pages cannot disagree. Dan supplied the
// Companies House record on 23 September 2026; the company name/number/registered
// office were verified there. The App Store seller remains his individual account.
test("both legal pages name the same provider and carry the same date", () => {
  const provider = (page) =>
    documentText(page).match(/provided by (.+?) \(“Snaglist”/)?.[1];
  const version = (page) =>
    html(page)
  ...
  for (const [label, read] of [
    ...
    assert.equal(
      terms,
      privacy,
      `${label} differs between /privacy and /terms`,
    );
  }
  for (const page of legalPages) {
    assert.equal(
      ...
      `${page}: verified company identity`,
    );
    assert.ok(
      documentText(page).includes(
        "registered office is 66 Paul Street, London, EC2A 4NA, United Kingdom",
      ),
      `${page}: registered office must be correctly labelled`,
    );
  }
});
...
test("the terms carry the Contractor link section for people without an account", () => {
  ...
  for (const fact of [
    ...
    assert.ok(
      text.includes(fact),
      `missing from /terms#contractor-links: ${fact}`,
    );
  assert.ok(
    html("/privacy").includes('href="/terms#contractor-links"'),
    "/privacy does not link the Contractor link terms",
  );
});
...
test("the terms use fair wording, keep consumer rights and invent no liability cap", () => {
  ...
  for (const [pattern, reason] of [
    ...
    [
      /not liable for any loss arising from service downtime/i,
      "the downtime exclusion",
    ],
    [
      /indirect, incidental, special, consequential/i,
      "the sweeping indirect-loss exclusion",
    ],
    [
      /continued use[^.]*constitutes acceptance/i,
      "continued use as acceptance of any change",
    ],
...
test("governing law stays a placeholder until Dan decides it (L2)", () => {
  const text = documentText("/terms");
  assert.ok(
    text.includes("[DAN: L2: governing law and courts]"),
    "governing-law placeholder missing",
  );
  assert.ok(
    !/Ireland/.test(text),
    "the hard-coded Ireland governing law has come back",
  );
});

test("online draft remains visibly unfinished and refuses an indexable production render", () => {
  const unresolved = legalPages.flatMap((page) =>
    [...html(page).matchAll(/\[DAN: ([^\]]*)\]/g)].map(
      ([, text]) => `${page}: ${text}`,
    ),
  );
  assert.ok(
    unresolved.length > 0,
    "Settle the online-service decisions and update its release review before activation",
  );
  const before = process.env.INDEX_PUBLIC_SITE;
  try {
    process.env.INDEX_PUBLIC_SITE = "true";
    assert.throws(
      () => render(),
      /Unresolved legal placeholder in a production build/,
    );
  } finally {
    if (before === undefined) delete process.env.INDEX_PUBLIC_SITE;
    else process.env.INDEX_PUBLIC_SITE = before;
  }
});
